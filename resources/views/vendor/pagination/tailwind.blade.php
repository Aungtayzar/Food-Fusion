...
>
  ...
  <span class="px-4 py-2 bg-orange-400 text-white rounded-l-lg">Previous</span>
  @else
  <a
    href="{{ $paginator->previousPageUrl() }}"
    rel="prev"
    class="px-4 py-2 bg-orange-500 text-white rounded-l-lg hover:bg-orange-600"
    >Previous</a
  >
  ...
  @foreach ($elements as $element) 
    {{--"Three Dots" Separator --}} 
    @if (is_string($element))
        <span class="px-4 py-2 bg-orange-300 text-orange-500">{{ $element }}</span>
    @endif 
    {{-- Array Of Links --}} 
    @if (is_array($element)) 
      @foreach ($element as $page => $url) 
        @if ($page == $paginator->currentPage())
          <span class="px-4 py-2 bg-orange-500 text-white ">{{ $page }}</span>
        @else
          <a
            href="{{ $url }}"
            class="px-4 py-2 bg-orange-200 text-orange-700 hover:bg-blue-600 hover:text-white"
            >{{ $page }}</a
          >
        @endif 
      @endforeach 
  ...
  @if( $paginator->hasMorePages())
    ...
    >
  @else
    <span class="px-4 py-2 bg-orange-300 text-orange-500 rounded-r-lg">Next</span>
  @endif
</nav>
@endif